...
class Gui extends OpenXum.Gui {
    constructor(c, e, l, g) {
        ...
        //this._selected_color = Sixth.Color.NONE;
            ...
            if (letter !== 'X') {
                ...
                if (number !== -1) {
                    ...
                    if (this._engine.phase() === Sixth.Phase.PUT_PIECE) {
                        ...
                        if (this._engine._verify_moving(letter, number)){
                            this._selected_piece = new Sixth.Coordinates(letter, number);
                            this._possible_moving_list = this._engine.get_possible_move_list(this._selected_piece);
                            this.draw();
                            ok = true;
                        }
                    }
    ...
    _draw_state() {
        ...
        for(let i = 0; i<this._engine._tabPiece.length; ++i)
        {
            ...
            if (intersection.state() === Sixth.State.VACANT) {
                this._draw_piece(letter, number, color, intersection.sixth());
            }
    ...
    _draw_piece(letter, number, color, sixth) {
        ...
        this._context.translate(0,longitude/2);

        if (color === Sixth.Color.BLACK) {
        ...
        this._context.beginPath();
        this._context.arc(latitude, longitude + 50, this._deltaX * (1.0 / 3 + 1.0 / 10), 2 * Math.PI, false);
        this._context.closePath();
        this._context.fill();
        this._context.stroke();
        ...
        this._context.beginPath();
        this._context.arc(latitude, longitude +50, this._deltaX * (1.0 / 3 + 1.0 / 10), 0.0, 2 * Math.PI);
        this._context.closePath();
        this._context.stroke();
    ...
    _draw_possible_move() {
        console.log("test");
        for (let i = 0; i < this._engine.get_possible_moving_list.length; ++i) {
            const x = this._offsetX + this._deltaX / 2 + this._engine._possible_move_list[i].x * this._deltaX;
            const y = this._offsetY + this._deltaY / 2 + this._engine._possible_move_list[i].y * this._deltaY;

            console.log(" const x : ", x);

            this._context.beginPath();
            this._context.lineWidth = 2;
            this._context.strokeStyle = this._engine.current_color === Sixth.Color.BLACK ? "#1187FF" : "#1187FF";
            this._context.fillStyle = this._engine.current_color === Sixth.Color.BLACK ? "#1187FF" : "#1187FF";
