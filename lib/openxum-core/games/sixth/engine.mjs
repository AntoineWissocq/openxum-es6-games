    ...
    move(move) {
        ...
        if (move.constructor === Array) {
            if (move[0].type() === MoveType.PUT_PIECE) {
                this._put_piece(move[0].coordinates(), this._current_color, 1, stack);
            }
        } else {
            if (move.type() === MoveType.PUT_PIECE) {
                this._put_piece(move.coordinates(), this._current_color, 1, stack);
            }
        }
...
/*
    ...
    */
    ...
    _move_piece(corigine,stack,Alevel,nbPiece,cdestination){
        for(let i=0; i<this._tabPiece.length; ++i){
            if(cdestination ===this._tabPiece[i]._coordinates){
                this._tabPiece[i]._level+=nbPiece;
                for(let j=stack.length-nbPiece; j<stack.length; ++j){
                    this._tabPiece[i]._stack[j].push(stack[j]);
                }
                break;
            }
        }
    ...
    _push_piece(origin, destination, stack, level) {
